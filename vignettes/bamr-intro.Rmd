...
devtools::install_github("markwh/bamr")
library("bamr")
```
...
As this `plot` method for `bamdata` returns a ggplot object, it can be modified, for example to make the y-axis be log scale:
...
(The `minmax` function used in the "upperbound_logQ" parameter takes the minimum in space of the maximum in time for each cross-section.)
...
Data-dependent priors can also be specified using quoted expressions. These must reference an object called "bamdata". For example:
